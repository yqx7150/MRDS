...
class DenoisingModel(BaseModel):
    ...
    #          ori = None, data_ob = None, mask = None,model=None, mode = 'test'):
    #     sde.set_mu(self.condition)
    ...
    #                                             ori = ori, mask = mask, model=model)
    #         elif mode == 'test_save':
    #             self.output, data_save = sde.reverse_sde(self.state, save_states=save_states,
    #                                             ori = ori, mask = mask, model=model)
    #         else:
    #             self.output,_ = sde.reverse_sde(self.state, save_states=save_states)
    ...
    def get_current_visuals(self, need_GT=True):
        ...
        out_dict["Output"] = torch.tensor(self.output).float().cpu()
        if need_GT:
            out_dict["GT"] = self.state_0.detach()[0].float().cpu()
        return out_dict
    def get_current_visuals_experiment(self, ):
        out_dict = OrderedDict()
        out_dict["Input"] = self.condition.detach()[0].float().cpu()
        out_dict["Output"] = torch.tensor(self.output).float().cpu()
